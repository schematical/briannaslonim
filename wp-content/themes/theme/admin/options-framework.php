...
function optionsframework_setdefaults() {
	...
	generate_options_css($options); //generate static css/js file
	generate_options_js($options);
...
function optionsframework_page() {
   ...
   <h2 class="themeoptions"> Theme Options</h2>
		<div class="extern-links">
			...
			<a class="icon-a icon-documentation" href='<?php echo get_template_directory_uri()?>/theme/pdf/documentation.pdf' target="_blank" ><span class="icon"><span>Documentation</span></span></a>
		</div>
		<br class="clear" /> 
...
function optionsframework_validate( $input ) {
	...
	if ( isset( $_POST['update'] ) ) {
		...
		generate_options_css($clean); //generate static css file
		generate_options_js($clean);
		return $clean;
	}
...
function of_get_default_values() {
	...
	generate_options_css($output); //generate static css file
...
function generate_options_css($newdata) {
	...
	include($css_dir . 'style.php'); // Generate CSS
	$css = ob_get_clean(); // Get generated CSS (output buffering)
	file_put_contents($css_dir . 'custom.css', $css, LOCK_EX); // Save it
	
}

function generate_options_js($newdata1) {
	$data = $newdata1;

	$js_dir = get_template_directory(). '/js/';
	ob_start(); // Capture all output (output buffering)
	
	include($js_dir . 'custom.js.php'); // Generate JS
	$js = ob_get_clean(); // Get generated JS (output buffering)
	file_put_contents($js_dir . 'custom.js', $js, LOCK_EX);// Save it
}